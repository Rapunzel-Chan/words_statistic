...
async def export_report(
    file: UploadFile = File(...), word_service: WordStatisticsService = Depends(get_word_statistics_service)
):
    """
    ...
    try:
        ...
        if file_size > settings.max_file_size:
            raise HTTPException(
                status_code=413, detail=f"File too large. Max size: {settings.max_file_size / 1024 / 1024:.0f}MB"
            )
        ...
        return ReportExportResponse(
            task_id=task_id, status="processing", message="File is being processed. Use task_id to check status."
        )
...
async def get_export_status(task_id: str, word_service: WordStatisticsService = Depends(get_word_statistics_service)):
    """
    ...
    if result.status == "completed":
        ...
        return {"task_id": result.task_id, "status": result.status, "error": result.error}
    else:
